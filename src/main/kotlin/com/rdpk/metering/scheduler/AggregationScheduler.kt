...
import com.rdpk.metering.service.AggregationProcessingService
 ...
 * Delegates business logic to AggregationProcessingService
...
class AggregationScheduler(
    ...
    private val aggregationProcessingService: AggregationProcessingService,
    ...
    fun processAggregationWindows() {
        ...
        resilienceService.applyPostgresResilience(
            ...
            .flatMap { pair ->
                ...
                processWindowWithLock(tenant.id!!, customer.id!!, windowStart, windowEnd)
            }
            .then()
            .subscribe(
                {},
    ...
    /**
     * Process window with distributed lock
     * Handles lock acquisition, metrics, and error handling
     */
    private fun processWindowWithLock(
        tenantId: Long,
        customerId: Long,
        windowStart: Instant,
        windowEnd: Instant
    ): Mono<Void> {
        val lockKey = "aggregation:lock:$tenantId:$customerId:${windowStart.epochSecond}"
        val lockSample = Timer.start()
        
        return distributedLockService.withLock<Void>(
            lockKey = lockKey,
            timeout = Duration.ofSeconds(lockTimeoutSeconds),
            leaseTime = Duration.ofSeconds(lockLeaseTimeSeconds),
            operation = aggregationProcessingService.processWindow(tenantId, customerId, windowStart, windowEnd)
        )
            .doOnSuccess {
            ...
            .doOnError { error ->
                ...
                log.error("Error processing window for tenant $tenantId, customer $customerId", error)
            }
            .onErrorResume { error ->
                log.error("Error processing window for tenant $tenantId, customer $customerId", error)
                Mono.empty<Void>()
